...
std::string Converter::MakeEncoderLayer(
  ...
  auto Q_T = builder->Transpose(name + "/mha/Q/transpose", Q, {0, 2, 1, 3});
  flow = builder->MatMul(
...
std::string Converter::MakeAttentionBody(OnnxBuilder* builder,
  ...
  if (weights.ip_mult_gate.size() > 0 || weights.ip_add_gate.size() > 0) {
    flow = builder->Reshape("/attn_body/ma_gating/reshape", flow,
                            {-1, 64, embedding_size});
  ...
  flow = builder->Reshape("/attn_body/rehape", flow, {-1, embedding_size});
