...
void SearchWorker::ExtendNode(NodeToProcess& picked_node) {
  ...
  picked_node.tt_low_node = search_->tt_->Lookup(picked_node.hash);
...
void SearchWorker::DoBackupUpdateSingleNode(
  ...
  if (node_to_process.nn_queried) {
    auto tt_low_node = search_->tt_->TryInsert(
        node_to_process.hash, std::move(node_to_process.tt_low_node));
